...
class Console {
 ...
 public:
  ...
  template <typename... Args>
  void AddTaggedMessage(std::string_view tag, std::string_view fmt,
                        Args&&... args) {
    std::string messageString =
        std::vformat(fmt, std::make_format_args(args...));
  ...
  template <typename... Args>
  void AddMessageWithColor(uint32_t color, std::string_view fmt,
                           Args&&... args) {
    std::string messageString =
        std::vformat(fmt, std::make_format_args(args...));
  ...
  template <typename... Args>
  void AddItalicMessageWithColor(uint32_t color, std::string_view fmt,
                                 Args&&... args) {
    std::string messageString =
        std::vformat(fmt, std::make_format_args(args...));
