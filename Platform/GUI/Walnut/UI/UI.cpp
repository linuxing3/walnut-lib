...
void DrawButtonImage(const std::shared_ptr<Walnut::Image> &imageNormal,
                     const std::shared_ptr<Walnut::Image> &imageHovered,
                     const std::shared_ptr<Walnut::Image> &imagePressed,
                     ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed,
                     ImVec2 rectMin, ImVec2 rectMax) {
  auto *drawList = ImGui::GetForegroundDrawList();
  if (ImGui::IsItemActive())
    drawList->AddImage(imagePressed->GetDescriptorSet(), rectMin, rectMax,
                       ImVec2(0, 0), ImVec2(1, 1), tintPressed);
  else if (ImGui::IsItemHovered())
    drawList->AddImage(imagePressed->GetDescriptorSet(), rectMin, rectMax,
                       ImVec2(0, 0), ImVec2(1, 1), tintHovered);
  else
    drawList->AddImage(imagePressed->GetDescriptorSet(), rectMin, rectMax,
                       ImVec2(0, 0), ImVec2(1, 1), tintNormal);
};
...
void DrawButtonImage(const std::shared_ptr<Walnut::Image> &imageNormal,
                     ...
                     ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed,
                     ImRect rectangle) {
  DrawButtonImage(imageNormal, imageHovered, imagePressed, tintNormal,
                  tintHovered, tintPressed, rectangle.Min, rectangle.Max);
};
...
void DrawButtonImage(const std::shared_ptr<Walnut::Image> &imageNormal,
                     ...
                     ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed) {
  DrawButtonImage(imageNormal, imageHovered, imagePressed, tintNormal,
                  tintHovered, tintPressed, ImGui::GetItemRectMin(),
...
void RenderWindowOuterBorders(ImGuiWindow *window) {
  ...
  auto GetResizeBorderRect = [](ImGuiWindow *window, int border_n,
...
bool UpdateWindowManualResize(ImGuiWindow *window, ImVec2 &newSize,
  ...
  auto CalcWindowSizeAfterConstraint = [](ImGuiWindow *window,
    ...
    if (!(window->Flags &
      ...
      ImGuiWindow *window_for_height =
          (window->DockNodeAsHost && window->DockNodeAsHost->VisibleWindow)
  ...
  auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](
                                   ImGuiWindow *window,
    ...
    ImVec2 size_pad{window->WindowPadding.x * 2.0f,
                    window->WindowPadding.y * 2.0f};
    ImVec2 size_desired = {size_contents.x + size_pad.x + 0.0f,
                           size_contents.y + size_pad.y + decoration_up_height};
    if (window->Flags & ImGuiWindowFlags_Tooltip) {
      ...
      ImVec2 size_auto_fit = ImClamp(
          ...
          ImMax(size_min,
                {avail_size.x - style.DisplaySafeAreaPadding.x * 2.0f,
                 avail_size.y - style.DisplaySafeAreaPadding.y * 2.0f}));

      // When the window cannot fit all contents (either because of constraints,
      // either because screen is too small), we are growing the size on the
      // other axis to compensate for expected scrollbar. FIXME: Might turn
  ...
  auto CalcResizePosSizeFromAnyCorner = [CalcWindowSizeAfterConstraint](
                                            ImGuiWindow *window,
    ...
    ImVec2 pos_max =
        ImLerp({window->Pos.x + window->Size.x, window->Pos.y + window->Size.y},
               corner_target, corner_norm); // Expected window lower-right
  ...
  // Calculate the range of allowed position for that window (to be movable and
  // visible past safe area padding) When clamping to stay visible, we will
  // enforce that window->Pos stays inside of visibility_rect.
  ImRect viewport_rect(window->Viewport->GetMainRect());
  ImRect viewport_work_rect(window->Viewport->GetWorkRect());
  ImVec2 visibility_padding =
      ImMax(style.DisplayWindowPadding, style.DisplaySafeAreaPadding);
  ImRect visibility_rect({viewport_work_rect.Min.x + visibility_padding.x,
                          viewport_work_rect.Min.y + visibility_padding.y},
                         {viewport_work_rect.Max.x - visibility_padding.x,
                          viewport_work_rect.Max.y - visibility_padding.y});
  ...
  for (int resize_grip_n = 0; resize_grip_n < resize_grip_count;
    ...
    bool hovered, held;
    const ImVec2 min = {corner.x - def.InnerDir.x * grip_hover_outer_size,
                        corner.y - def.InnerDir.y * grip_hover_outer_size};
    const ImVec2 max = {corner.x + def.InnerDir.x * grip_hover_outer_size,
                        corner.y + def.InnerDir.y * grip_hover_outer_size};
    ...
    ImGui::ButtonBehavior(resize_rect, resize_grip_id, &hovered, &held,
    ...
    if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
      ...
      const float x =
          ...
          ImLerp(def.InnerDir.x * grip_hover_outer_size,
                 def.InnerDir.x * -grip_hover_inner_size, def.CornerPosN.x);
      const float y =
          g.IO.MousePos.y - g.ActiveIdClickOffset.y +
          ImLerp(def.InnerDir.y * grip_hover_outer_size,
                 def.InnerDir.y * -grip_hover_inner_size, def.CornerPosN.y);

      ImVec2 corner_target(
          x, y); // Corner of the window corresponding to our corner grip
    ...
    if (resize_grip_n == 0 || held || hovered)
      resize_grip_col[resize_grip_n] =
          ImGui::GetColorU32(held      ? ImGuiCol_ResizeGripActive
  ...
  for (int border_n = 0; border_n < resize_border_count; border_n++) {
    ...
    ImGui::ButtonBehavior(border_rect, border_id, &hovered, &held,
                          ImGuiButtonFlags_FlattenChildren);
...
bool BeginMenubar(const ImRect &barRectangle) {
  ...
  ImGui::BeginGroup(); // Backup position on layer 0 // FIXME: Misleading to use
  ...
  // glitchy.
  ImRect bar_rect =
      UI::RectOffset(barRectangle, 0.0f, padding.y); // window->MenuBarRect();
  ImRect clip_rect(
      IM_ROUND(ImMax(window->Pos.x, bar_rect.Min.x + window->WindowBorderSize +
                                        window->Pos.x - 10.0f)),
...
void EndMenubar() {
  ...
  if (ImGui::NavMoveRequestButNoResultYet() &&
    ...
    if (nav_earliest_child->ParentWindow == window &&
      ...
      ImGui::NavMoveRequestForward(g.NavMoveDir, g.NavMoveClipDir,
                                   g.NavMoveFlags,
  ...
  // IM_ASSERT(window->Flags & ImGuiWindowFlags_MenuBar); // NOTE(Yan): Needs to
  // be commented out because Jay
  IM_ASSERT(window->DC.MenuBarAppending);
  ImGui::PopClipRect();
  ImGui::PopID();
